import { FunctionReturn, getChainFromName, getChainName, toResult, FunctionOptions, TransactionParams } from '@heyanon/sdk';
import { Address, getContract, encodeFunctionData } from 'viem';

import { LBRouterV22ABI, LB_ROUTER_V22_ADDRESS, LBPairV21ABI, PairV2 } from '@traderjoe-xyz/sdk-v2';
...
export async function removeLiquidity({ chainName, tokenX, tokenY, account, binStep }: Props, { getProvider, sendTransactions }: FunctionOptions): Promise<FunctionReturn> {
    ...
    if (tokenX == tokenY) return toResult('Parameters `tokenX` and `tokenY` cannot be the same', true);
    ...
    const range = 200;
    const addressArray = Array.from<Address>({ length: 2 * range + 1 }).fill(account);
    ...
    const isApproved = await provider.readContract({
        ...
        args: [account, LB_ROUTER_V22_ADDRESS[chainId]],
    });
    ...
    if (!isApproved) {
        const tx: TransactionParams = {
            ...
            data: encodeFunctionData({
                ...
                args: [LB_ROUTER_V22_ADDRESS[chainId], true],
            }),
        };
