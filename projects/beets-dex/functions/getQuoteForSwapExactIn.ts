...
interface Props {
    ...
    humanReadableSwapAmount: string;
}

export async function getQuoteForSwapExactIn(
    { chainName, tokenInAddress, tokenOutAddress, humanReadableSwapAmount }: Props,
    ...
    if (!validateTokenPositiveDecimalAmount(humanReadableSwapAmount)) return toResult(`Invalid swap amount: ${humanReadableSwapAmount}`, true);
    ...
    notify(`Getting quote for swap ${humanReadableSwapAmount} ${balancerTokenIn.symbol} -> ${balancerTokenOut.symbol} on ${chainName}`);
    ...
    const balancerSwapAmount = TokenAmount.fromHumanAmount(balancerTokenIn, humanReadableSwapAmount as `${number}`);
