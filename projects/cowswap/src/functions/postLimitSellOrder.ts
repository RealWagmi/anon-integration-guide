...
export async function postLimitSellOrder(
  ...
  const {
    evm: { getProvider, sendTransactions, signMessages },
    notify,
  } = options;
  ...
  // Validate chain
  const chainId = getChainFromName(chainName as EvmChain) as SupportedChainId;
  ...
  const buyTokenAmountBN =
    (buyTokenPriceBN / sellTokenPriceBN) * sellTokenAmountBN;
  const buyTokenAmount = parseUnits(
    formatUnits(buyTokenAmountBN, sellTokenInfo.decimals),
    buyTokenInfo.decimals,
  );

  const approval: EVM.types.TransactionParams[] = [];
  ...
  const slippageToleranceToBipsResult =
    slippageToleranceToBips(slippageTolerance);
