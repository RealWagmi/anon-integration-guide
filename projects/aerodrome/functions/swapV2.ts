...
import { CommandCode, MIXED_QUOTER_ADDRESS, Route, UNIVERSAL_ROUTER_ADDRESS, V2SwapExactIn, } from "../constants";
import { parseAmount, parseTokensAndStables, parseWallet } from "../utils/parse";
import { encodeV2SwapExactIn } from "../utils/encode";
import { encodePathV2 } from "../utils/path";
...
type Props = {
    ...
    tokens: Address[];
    /**
     * @description List of flags denoting if given pool is stable. The stables are ordered and apply
     * to each consecutive token pair. `stables[0]` would apply to a pair of `tokens[0]` and `tokens[1]`
     */
    stables: boolean[];
};
 ...
 */
export async function swapV2(props: Props, { sendTransactions, notify, getProvider }: FunctionOptions): Promise<FunctionReturn> {
    // get wallet
    ...
    // get tokens and fees
    const tokensAndStables = parseTokensAndStables(props);
    if (!tokensAndStables.success) {
        return toResult(tokensAndStables.errorMessage, true);
    }
    const { tokens, stables } = tokensAndStables.data;

    // build route and path
    let routes: Route[] = [];
    ...
    let path = encodePathV2(tokens, stables);
    ...
    let encodedInputs: Hex[] = [encodeV2SwapExactIn(swap)];
    ...
    // return
    return toResult(result.isMultisig ? message.message : `Successfully executed swap using Aerodrome V2 through Universal Router. ${message.message}`);
}
