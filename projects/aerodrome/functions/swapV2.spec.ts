...
import { CommandCode, UNIVERSAL_ROUTER_ADDRESS, V2SwapExactIn } from "../constants";
...
describe('swapV2', () => {
    it('should swap wETH for AERO using V2', async () => {
        const props: Parameters<typeof swapV2>[0] = {
        ...
        const tools: Parameters<typeof swapV2>[1] = {
            sendTransactions: vi.fn().mockReturnValue(Promise.resolve({ data: ['Result'], isMultisig: false })),
            notify: vi.fn(),
            getProvider: vi.fn().mockReturnValue(provider),
        };

        const result = await swapV2(props, tools);

        // build command and input bytecode
        let commandBytecode: Hex = '0x08';
        let encodedInputs: Hex[] = [encodeV2SwapExactIn(swapObj)];
        ...
        expect(result.data).toMatch(`Successfully executed swap using Aerodrome V2 through Universal Router.`);
    });
});
