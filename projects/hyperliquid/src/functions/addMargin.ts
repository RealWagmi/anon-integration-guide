...
/**
 * Adds margin to the perp position so it is safer from liquidation
 * @param account - User's wallet address
 * @param asset - The position that user wants to modify the margin for
 * @param amount - The amount of USD to add to the margin
 ...
 */
export async function addMargin({ account, asset, amount, vault }: Props, options: FunctionOptions): Promise<FunctionReturn> {
    const {
        notify,
        evm: { signTypedDatas },
    } = options;
    try {
        ...
        if (usdAmount > withdrawable) {
            return toResult('Can not add margin, balance too low. ', true);
        }
        ...
        for (const { position } of assetPositions) {
            ...
            if (coin == asset) {
                ...
                return toResult(`Successfully added margin!`);
            }
        }
