...
/**
 * Creates a stop loss trigger order on the full position of the specified asset
 * @param account - User's wallet address
 * @param asset - The asset to trade on Hyperliquid.
 * @param price - Price that triggers the stop loss action
 ...
 */
export async function addStopLoss({ account, asset, price, vault }: Props, { evm: { signTypedDatas } }: FunctionOptions): Promise<FunctionReturn> {
    try {
        ...
        for (const { position } of assetPositions) {
            ...
            if (coin == asset) {
                ...
                return toResult(`Successfully created stop loss trigger!`);
            }
        }
        return toResult("You don't have position in that asset", true);
    } catch (error) {
        console.log(error);
        return toResult('Failed to create stop loss trigger on Hyperliquid. Please try again.', true);
    }
}
