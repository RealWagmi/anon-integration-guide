 ...
 */
export async function modifyPerpPositionByTokenAmount({ account, asset, size }: Props, options: FunctionOptions): Promise<FunctionReturn> {
        ...
        for (const { position } of assetPositions) {
            ...
            if (coin == asset) {
                ...
                const result = await openPerp(
                    { account, asset, size, sizeUnit: 'ASSET', leverage: leverage.value, short: Number(szi) > 0 ? false : true, updating: true },
                    options,
                );
                if (!result.success) {
