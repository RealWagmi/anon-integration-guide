 ...
 */
export async function closeVault({ account, vault }: Props, { evm: { signTypedDatas } }: FunctionOptions): Promise<FunctionReturn> {
    try {
        ...
        if (data?.assetPositions?.length) {
            return toResult(`Close vault's positions before closing it`, true);
        }
        ...
        return toResult(`Successfully closed vault!`);
    } catch (error) {
        console.log(error);
        return toResult('Failed to close vault on Hyperliquid. Please try again.', true);
    }
}
