...
import { transferToPerpetual } from '../transferToPerpetual';
...
describe('transferToPerpetual', () => {
    ...
    it('should prepare and send transfer transaction correctly', async () => {
        ...
        const result = await transferToPerpetual(props, functionOptions as any);

        expect(result.success).toBe(true);
        expect(result.data).toContain('Successfully initiated transfer of 100 USDC from spot to perp balance on Hyperliquid');
    ...
    it('should return error if amount is invalid', async () => {
        const result = await transferToPerpetual({ amount: 'invalid-amount' }, functionOptions as any);

        expect(result).toEqual(toResult('Invalid amount specified', true));
    });

    it('should return error if amount is negative', async () => {
        const result = await transferToPerpetual({ amount: '-10' }, functionOptions as any);

        expect(result).toEqual(toResult('Invalid amount specified', true));
    });

    it('should return error if amount is zero', async () => {
        const result = await transferToPerpetual({ amount: '0' }, functionOptions as any);
    ...
    it('should validate different amount formats', async () => {
        const testCases = [
            { amount: '1000', expectedSuccess: true, message: 'Successfully initiated transfer of 1000 USDC from spot to perp balance on Hyperliquid' },
            { amount: '1000.00', expectedSuccess: true, message: 'Successfully initiated transfer of 1000.00 USDC from spot to perp balance on Hyperliquid' },
            { amount: '1000.50', expectedSuccess: true, message: 'Successfully initiated transfer of 1000.50 USDC from spot to perp balance on Hyperliquid' },
            { amount: '0.1', expectedSuccess: true, message: 'Successfully initiated transfer of 0.1 USDC from spot to perp balance on Hyperliquid' },
            { amount: '.5', expectedSuccess: true, message: 'Successfully initiated transfer of .5 USDC from spot to perp balance on Hyperliquid' },
        ...
        for (const testCase of testCases) {
            ...
            const result = await transferToPerpetual({ amount: testCase.amount }, functionOptions as any);
    ...
    it('should log debug information', async () => {
        ...
        await transferToPerpetual(props, functionOptions as any);

        expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('Starting spot->perp transfer with:'), expect.any(Object));
    });
