...
describe('toggleAutoCloseOnWithdrawal', () => {
    beforeEach(() => {
        vi.clearAllMocks();
    });

    it('should successfully toggle the setting', async () => {
        mockedAxios.post
            .mockResolvedValueOnce({ data: { alwaysCloseOnWithdraw: false } }) // vaultDetails
        ...
        const result = await toggleAutoCloseOnWithdrawal(defaultProps, functionOptions as any);
    ...
    it('should return early if the setting is already correct', async () => {
        mockedAxios.post.mockResolvedValueOnce({ data: { alwaysCloseOnWithdraw: true } }); // vaultDetails
    ...
    it('should resolve vault name to address', async () => {
        mockedAxios.post
            .mockResolvedValueOnce({ data: { alwaysCloseOnWithdraw: false } })
        ...
        const result = await toggleAutoCloseOnWithdrawal({ ...defaultProps, vault: 'Sifu' }, functionOptions as any);

        expect(result.success).toBe(true);
    });

    it('should return error if vault name is invalid', async () => {
        const result = await toggleAutoCloseOnWithdrawal({ ...defaultProps, vault: 'InvalidName' }, functionOptions as any);
    ...
    it('should return error if signTypedDatas is missing', async () => {
        const result = await toggleAutoCloseOnWithdrawal(defaultProps, {
            evm: {},
        } as any);
    ...
    it('should handle error from vaultModify status: err', async () => {
        mockedAxios.post
            .mockResolvedValueOnce({ data: { alwaysCloseOnWithdraw: false } })
            .mockResolvedValueOnce({ data: { status: 'ok' } })
            .mockResolvedValueOnce({ data: { status: 'err', response: 'some failure' } });
    ...
    it('should handle error from response statuses', async () => {
        mockedAxios.post
            .mockResolvedValueOnce({ data: { alwaysCloseOnWithdraw: false } })
            .mockResolvedValueOnce({ data: { status: 'ok' } })
            .mockResolvedValueOnce({
    ...
    it('should catch and log unexpected errors', async () => {
        mockedAxios.post.mockRejectedValueOnce(new Error('Network issue'));
