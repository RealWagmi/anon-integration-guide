...
async function test() {
    ...
    console.log(`\nAttempting to remove ${amountToRemove} ALP for WETH...`);
    ...
    try {
        ...
        if (result.success) {
            ...
            // Additional information about the WETH received
            console.log('\nWETH Details:');
            console.log('WETH Address:', CONTRACT_ADDRESSES[NETWORKS.SONIC].WETH);
            console.log('Expected minimum WETH:', Number(details.details.minOut) / 1e18, 'WETH');
        } else {
