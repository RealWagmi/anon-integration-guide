...
export async function createTakeProfitStopLossOrder(
	{ market, side, amount, amountCurrency, takeProfitTriggerPrice, takeProfitLimitPrice, stopLossTriggerPrice, stopLossLimitPrice }: Props,
	...
	if (takeProfitTriggerPrice && !stopLossTriggerPrice) {
		try {
			amount = await convertToBaseAmount({ amount, amountCurrency, market, limitPrice: takeProfitLimitPrice || takeProfitTriggerPrice, exchange });
	...
	if (!takeProfitTriggerPrice && stopLossTriggerPrice) {
		try {
			amount = await convertToBaseAmount({ amount, amountCurrency, market, limitPrice: stopLossLimitPrice || stopLossTriggerPrice, exchange });
	...
	// Case of a take profit AND stop loss order (OCO)
		...
		try {
			amount = await convertToBaseAmount({ amount, amountCurrency, market, limitPrice: null, exchange });
