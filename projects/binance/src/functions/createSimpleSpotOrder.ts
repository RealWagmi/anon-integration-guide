 ...
 * @param {string|null} props.type - Type of the order, either "limit" or "market"
 * @param {string} props.side - Side of the order, either "buy" or "sell"
 * @param {number} props.amount - Amount of the order, for example 1
 * @param {number|null} props.price - Price of the order, for example 100000
...
export async function createSimpleSpotOrder({ market, type, side, amount, price }: Props, { exchange }: FunctionOptionsWithExchange): Promise<FunctionReturn> {
    // If type is not provided, assume it's a market order unless a price is provided
    if (type === null) {
    ...
    if (type === 'market' && price !== null) {
        return toResult('Market orders don\'t need a price', true);
    }
