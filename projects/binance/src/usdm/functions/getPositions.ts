 ...
 */
export async function getPositions({}: Props, { getCcxtExchange }: FunctionOptions): Promise<FunctionReturn> {
	try {
		...
		const rows = [
			...
			...mostRecentNPositions.map((position, index) => formatPositionMultiLine(position, markets[position.symbol] as MarketInterface, `${index + 1}. `)),
		];
