...
export function formatPositionMultiLine(position: Position, market?: MarketInterface, prefix: string = '', delimiter: string = '\n'): string {
	...
	const rows = [
		...
		`${side !== 'N/A' ? `${prefix}Side: ${side}` : ''}`,
		// `${prefix}Margin Ratio: ${marginRatio}`,
		`${entryPrice !== 'N/A' ? `${prefix}Entry Price: ${entryPrice}` : ''}`,
		`${markPrice !== 'N/A' ? `${prefix}Mark Price: ${markPrice}` : ''}`,
		`${liquidationPrice !== 'N/A' ? `${prefix}Liquidation Price: ${liquidationPrice}` : ''}`,
		`${notional !== 'N/A' ? `${prefix}Notional: ${notional}` : ''}`,
		`${unrealizedPnl !== 'N/A' ? `${prefix}PnL: ${unrealizedPnl}` : ''}`,
		`${unrealizedPnlPercentage !== 'N/A' ? `${prefix}PnL Percentage: ${unrealizedPnlPercentage}` : ''}`,
		`${uiMargin !== 'N/A' ? `${prefix}Margin: ${uiMargin}` : ''}`,
		`${position.marginMode === 'isolated' ? `${prefix}Collateral (margin + PnL): ${collateral}` : ''}`,
		`${marginMode !== 'N/A' ? `${prefix}Margin Mode: ${marginMode}` : ''}`,
		`${leverage !== 'N/A' ? `${prefix}Leverage: ${leverage}` : ''}`,
		`${expiry !== 'N/A' ? `${prefix}Expiry/Delivery date: ${expiry}` : ''}`,
		`${contracts !== 'N/A' ? `${prefix}Contracts: ${contracts}` : ''}`,
		`${contractSize !== 'N/A' ? `${prefix}Contract Size: ${contractSize}` : ''}`,
		`${timestamp !== 'N/A' ? `${prefix}Created: ${timestamp}` : ''}`,
	].filter(Boolean);
