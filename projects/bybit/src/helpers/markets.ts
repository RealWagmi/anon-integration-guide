import { Exchange, MarketInterface, MarketType as CcxtMarketType, Ticker } from 'ccxt';
...
/**
 * Given an exchange, a currency and a CCXT market type, get a list of all its
 * active markets for the given currency and market type.
 */
export async function getMarketsWithCurrencyAndType(currency: string, ccxtMarketType: CcxtMarketType, exchange: Exchange): Promise<MarketInterface[]> {
    const markets = await getMarketsWithCurrency(currency, exchange);
    const filteredMarkets = markets.filter((market) => market.type === ccxtMarketType);
...
/**
 * Given a CCXT market object, return the corresponding HeyAnon market type.
 *
 * @link https://docs.ccxt.com/#/?id=contract-naming-conventions
 */
export function fromCcxtMarketToMarketType(market: MarketInterface): (typeof MARKET_TYPES)[number] {
    switch (market.type) {
        case 'delivery':
        case 'margin':
        case 'spot':
        ...
        case 'future':
            ...
            return 'perpetual';
        default:
            throw new Error(`Market type (CCXT) not supported: ${market.type}`);
    }
}

/**
 * Given a HeyAnon market type, return the corresponding CCXT market type.
 */
export function toCcxtMarketType(marketType: (typeof MARKET_TYPES)[number]): CcxtMarketType {
    switch (marketType) {
        case 'margin':
        case 'spot':
        case 'option':
        ...
        case 'delivery':
            return 'future';
        default:
            throw new Error(`Market type (HeyAnon) not supported: ${marketType}`);
    }
}
